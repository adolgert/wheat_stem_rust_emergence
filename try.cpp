...
int main(int argc, char* argv[])
{
	...
	raster_faces gimms_faces(gimms_layer);
	raster_faces cdls_faces(cdls_layer);
	
	auto face_iter=gimms_faces.forward();
	while (face_iter[0]!=face_iter[1]) {
		auto gimms_face=*face_iter[0];
		raster_faces gc_faces=cdls_faces.intersect(gimms_face);
		auto gc_iter=gc_faces.forward();
		while (gc_iter[0]!=gc_iter[1]) {
			gc_iter[0]++;
		}
		face_iter[0]++;
	}
